...
在正式环境启用前，请在目标系统验证 Cloudflare 配额、Durable Object 迁移、Windows/macOS 服务生命周期、日志脱敏和实际 MCP 客户端兼容性。
...
架构、传输协议和迁移文件属于高级参考，供维护者在需要时查阅。法律文件、第三方声明和社区规则位于 [docs](docs/) 目录。
