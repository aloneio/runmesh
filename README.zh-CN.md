...
本预览版的 hosted bootstrap 默认关闭；只有精确 fixed signed release 已发布、独立验证、配置 canonical `RUNMESH_PUBLIC_ORIGIN` 并在 Worker 部署中显式确认后才会启用。在此之前请使用手工验证的 portable artifact；自动 Runner 更新、下载、数据 downgrade 和回滚不在当前预览版范围内。
