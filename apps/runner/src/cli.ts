...
import { installServiceManifest, isManagedService, removeServiceManifest, renderService, serviceCommands } from "./service.js";

export interface CliDependencies { readonly store?: ProfileStore; readonly stdout?: (line: string) => void; readonly stderr?: (line: string) => void; readonly fetch?: typeof globalThis.fetch; readonly startRunner?: (config: Awaited<ReturnType<typeof validateRunnerConfig>>) => Promise<void>; }
export interface EnrollCliDependencies { readonly store?: ProfileStore; readonly stdout?: (line: string) => void; readonly stderr?: (line: string) => void; readonly fetch?: typeof globalThis.fetch; }
...
export async function runEnrollCli(argv: readonly string[], dependencies: EnrollCliDependencies = {}): Promise<void> {
  ...
  try {
    ...
    report(output, parsed.json, { enrolled: true, runner_id: result.profile.runner_id, workspace: result.profile.workspaces.at(-1)?.id });
...
export async function runCli(argv: readonly string[], dependencies: CliDependencies = {}): Promise<void> {
  ...
  try {
    ...
    if (parsed.command === "doctor") { report(output, parsed.json, await doctor(store)); return; }
    if (parsed.command === "install" || parsed.command === "stop" || parsed.command === "restart") { await serviceCommand(parsed, store, output); return; }
    if (parsed.command === "uninstall") { await uninstall(parsed, store, output); return; }
...
async function start(parsed: ParsedCommand, store: ProfileStore, error: (line: string) => void, dependencies: CliDependencies): Promise<void> {
  ...
  const productWorkspaces = profile?.workspaces.length ? workspaceOptions(profile) : [{ workspaceId: defaultWorkspaceId(current), rootPath: current, readonly: false, shell: true }];
...
async function doctor(store: ProfileStore): Promise<Record<string, unknown>> {
  ...
  const manifest = renderService({ profilePath: store.filePath });
...
async function serviceCommand(parsed: ParsedCommand, store: ProfileStore, output: (line: string) => void): Promise<void> {
  const manifest = renderService({ profilePath: store.filePath });
  if (parsed.command === "install") await installServiceManifest(manifest);
  report(output, parsed.json, { action: parsed.command, manifest: manifest.path, commands: serviceCommands(parsed.command as "install" | "stop" | "restart", manifest.platform) });
}
async function uninstall(parsed: ParsedCommand, store: ProfileStore, output: (line: string) => void): Promise<void> {
  if (parsed.values.purge === true && parsed.values.yes !== true) throw new Error("--purge requires --yes");
  const manifest = renderService({ profilePath: store.filePath });
  const removed = await removeServiceManifest(manifest);
  ...
  report(output, parsed.json, { action: "uninstall", service_removed: removed, profile_removed: parsed.values.purge === true, commands: serviceCommands("uninstall", manifest.platform) });
}
...
export function parseProductArgs(argv: readonly string[]): ParsedCommand {
  ...
  if (command === "start") return { command, json: argv.includes("--json"), values, passthrough: argv.slice(1).filter((arg) => arg !== "--json") };
  ...
  for (let index = 0; index < rest.length; index += 1) {
    ...
    const key = arg === "--server" ? "server" : arg === "--code" ? "code" : arg === "--cwd" ? "cwd" : arg === "--id" ? "id" : arg === "--path" ? "path" : undefined;
...
async function currentWorkspace(): Promise<string> { return canonicalDirectory(process.cwd()); }
