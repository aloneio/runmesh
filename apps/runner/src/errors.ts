...
export function failureMetadata(code: string): RpcFailureMetadata {
  ...
  if (["baseline_changed", "expected_hash_mismatch", "hunk_ambiguous", "hunk_not_found", "hunk_overlap", "target_exists", "missing_file", "search_snapshot_changed", "request_id_conflict"].includes(code)) return { failure_class: "conflict", operation_state: "not_started", next_action: "re_read_and_retry" };
  if (["file_too_large", "git_output_too_large", "queue_full", "busy"].includes(code)) return (code === "busy" || code === "queue_full") ? { failure_class: "resource", operation_state: "not_started", retry_after_ms: 1_000, next_action: "wait_and_retry" } : { failure_class: "resource", operation_state: "not_started", next_action: "correct_request" };
