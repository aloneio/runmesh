...
export function failureMetadata(code: string): RpcFailureMetadata {
  ...
  if (["baseline_changed", "expected_hash_mismatch", "hunk_ambiguous", "hunk_not_found", "hunk_overlap", "target_exists", "missing_file", "search_snapshot_changed"].includes(code)) return { failure_class: "conflict", operation_state: "not_started", next_action: "re_read_and_retry" };
