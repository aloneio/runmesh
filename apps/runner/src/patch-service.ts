...
function changeReceipt(change: PlannedChange): { readonly path: string; readonly status: "updated" | "created" | "deleted"; readonly before_hash: string | null; readonly after_hash: string | null; readonly mode: number | null } {
  return {
...
function patchPreviewId(workspaceId: string, generation: number, patch: string, changes: readonly ReturnType<typeof changeReceipt>[]): string {
...
function changePreview(change: PlannedChange): Record<string, unknown> & { insertions: number; deletions: number } {
