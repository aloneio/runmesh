...
export function secretCreatedPage(title: string, url: string, clientId?: string): string {
  const next = clientId === undefined ? "" : `<section class="central-next-step"><h2>Use your shared capabilities</h2><p>Copy this connection URL into your AI client. All enabled services and Skills are shared automatically.</p><a class="button" href="/admin/central">Browse shared library</a><p class="muted">In your AI client, add a remote MCP connection and paste the URL. Refresh its tools after publishing changes.</p></section>`;
