...
export function clientDetailPage(client: Record<string, unknown>, runners: readonly RunnerSummaryViewModel[], overrides: readonly Record<string, unknown>[], csrf: string): string {
  ...
  if (scopeValues.length === 0) return '<section class="page-heading"><div><p class="eyebrow">AI connection</p><h1 data-no-i18n>' + escapeHtml(label) + '</h1><p>Services and Skills only - no Runner needed</p></div><a class="button secondary" href="/admin/clients">Back to clients</a></section>'
    + '<section class="panel"><h2>Available capabilities</h2><p>Choose approved services and Skills in the capability library. This connection has no computer permissions.</p><a class="button" href="/admin/central?client=' + encodeURIComponent(clientId) + '">Manage client access</a><p>' + clientCredentialBadge(isRevoked) + '</p></section>';
  ...
  return `<section class="detail-header">
    <div class="detail-title-group">
      ...
      <p class="lede">${message("text.runner.specific.access.can.only.further.restrict.the.client.s.global.scopes.it.can.never.g", "en")}</p><p class="muted">${message("client.credential.explanation", "en")}</p>
    </div>
