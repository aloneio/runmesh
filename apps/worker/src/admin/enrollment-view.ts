...
export function enrollmentDocument({ publicBase, runnerId, code, csrf, reEnroll, bootstrap, executionMode, maxValidityDays, enrollment }: EnrollmentView): string {
  ...
  const modeLabel = executionMode === "privileged_host" ? "Full host control (privileged_host)" : "Restricted service account (dedicated_user)";
...
if ($LASTEXITCODE -ne 0) { throw 'Runner doctor check failed.' }`,
  ...
  // Separate authored messages from interpolated values so each text node
  // is translated once. Commands, credential material and IDs stay opaque.
  const instruction = commands === manualCommands
    ? "Manual Runner enrollment and install uses a verified portable artifact. Install the artifact first, then run the commands below. It will ask for this code locally; paste it and press Enter."
    : "The installer verifies the fixed signed Runner artifact, downloads and verifies a private Node.js runtime for the host architecture, registers the Runner as a background service, and starts it after enrollment. The copied command includes the one-time enrollment code; no second code entry is needed. Treat the command as a secret.";
  const modeGuidance = "The default is dedicated_user; privileged_host is an advanced, explicitly confirmed option.";
  const warningBlock = executionMode === "privileged_host" ? `<p class="warning privileged-host-warning"><span>${escapeHtml(privilegedWarning)}</span> <span>${message("text.you.must.keep.the.one.time.confirmation.in.the.local.install.command", "en")}</span></p>` : `<p class="notice">${message("text.selected.restricted.service.account.mode.dedicated.user.the.installer.preserves.this.selec", "en")}</p>`;
  ...
  const removalBlock = `<details class="panel"><summary><strong>${message("text.remove.this.runner.from.the.host", "en")}</strong></summary><p class="muted font-12">${message("text.run.the.command.for.the.local.os.to.stop.the.managed.service.and.remove.the.runmesh.instal", "en")}</p><p><strong>Linux / macOS</strong></p><pre><code>${escapeHtml(removalCommands.linux)}</code></pre><p><strong>${message("text.windows.powershell.administrator", "en")}</strong></p><pre><code>${escapeHtml(removalCommands.windows)}</code></pre></details>`;

  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><link rel="icon" href="/assets/favicon.png" type="image/png"><title>Runmesh · Agent Control Plane enrollment</title>${adminStyles()}</head><body class="ops-body enrollment-body"><a class="skip-link" href="#main-content">${message("text.skip.to.main.content", "en")}</a>${controlHeader("runners")}<main class="shell enrollment-shell" id="main-content" tabindex="-1"><dialog open aria-labelledby="enrollment-title" class="enrollment-dialog"><section class="page-heading"><div><p class="eyebrow">${title}</p><h1 id="enrollment-title">${message("text.enroll.runner", "en")}</h1><p class="lede"><span>${escapeHtml(instruction)}</span> <span>${message("text.selected.execution.mode.2", "en")}</span> <span>${escapeHtml(modeLabel)}</span>. <span>${modeGuidance}</span> <span>${message("text.the.install.step.runs.only.after.enrollment.succeeds", "en")}</span> <span>${escapeHtml(enrollmentSummary)}</span></p></div></section><div class="enrollment-meta-box"><span class="form-stat-label">${message("text.target.runner.id", "en")}</span><span class="mono"><span data-no-i18n>${escapeHtml(runnerId)}</span></span></div><div class="enrollment-meta-box"><span class="form-stat-label">${message("text.selected.execution.mode", "en")}</span><span class="mono">${escapeHtml(modeLabel)}</span></div><div class="enrollment-meta-box"><span class="form-stat-label">${message("text.one.time.enrollment.code", "en")}</span><code class="mono" data-no-i18n>${escapeHtml(code)}</code><span class="muted font-12">${commands === manualCommands ? "Paste it only into the local prompt after verification; it is deliberately excluded from copied commands." : "The copied command includes this one-time code. Treat it as a secret and use it only once."}</span></div><div role="tablist" aria-label="Operating system" class="tabs">${tabs}</div><div class="enrollment-command-panels">${panels}</div>${warningBlock}<p class="warning">${message("text.do.not.share.this.code.it.is.single.use.enrollment.material.not.an.administrator.password", "en")}</p>${removalBlock}<div class="top-actions dialog-actions"><form method="post" action="/admin/runners/${encodeURIComponent(runnerId)}/enrollment">${executionModeFormFields(executionMode, csrf)}${windowFields("code", maxValidityDays)}<button class="button secondary">${message("text.regenerate.enrollment", "en")}</button></form><a class="button" href="/admin/runners">${message("text.done", "en")}</a></div></dialog></main>${adminScript()}</body></html>`;
}
