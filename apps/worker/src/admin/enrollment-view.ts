...
if ($LASTEXITCODE -ne 0) { throw 'Runner doctor check failed.' }`,
  ...
  const warningBlock = executionMode === "privileged_host" ? `<p class="warning privileged-host-warning"><span>${escapeHtml(privilegedWarning)}</span> <span>You must keep the one-time confirmation in the local install command.</span></p>` : `<p class="notice">Selected restricted service account mode: dedicated_user. The installer preserves this selection.</p>`;
