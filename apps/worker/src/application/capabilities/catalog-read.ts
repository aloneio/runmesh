...
export function createCatalogReader(ports: CatalogReadPorts) {
  return async (principal: CapturedIdentity, input: unknown, signal: AbortSignal, expired: () => boolean): Promise<CatalogPage> => {
    ...
    try {
      ...
      const rawGrant = ports.grant(principal.client_id);
      if (rawGrant === undefined) return { state: "denied" };
      const grant = parseCapabilityGrant(rawGrant);
      if (grant === undefined) return { state: "unavailable" };
      if (!grant.enabled || grant.client_id !== principal.client_id
        || !grant.rules.some(rule => rule.kind === "remote_tool" && rule.connection_profile_id === query.profile_id)) return { state: "denied" };
      // Unauthorized callers do not even probe the profile/catalog owner.
      ...
      if (query.cursor !== undefined && (cursor === undefined || cursor.client_id !== principal.client_id || cursor.secret_version !== principal.secret_version
        ...
        || cursor.grant_revision !== grant.revision || cursor.limit !== limit || cursor.expires_at_ms <= now
      ...
      if (head.approved_digest !== null) {
        ...
        tools = compatibleApprovedTools(observed, approved, head.approved_names).filter(tool => grantAllows(grant, principal.client_id,
          { kind: "remote_tool", resource_id: tool.tool_id, version: tool.version, connection_profile_id: profile.profile_id }));
      ...
      const next = nextOffset >= tools.length ? null : await ports.cursor.seal({ schema_version: 1, client_id: principal.client_id,
        secret_version: principal.secret_version, profile_id: profile.profile_id, profile_revision: profile.revision, grant_revision: grant.revision,
      ...
      const grantRecord = ports.grant(principal.client_id), profileRecord = ports.profile(profile.profile_id);
      if (grantRecord === undefined || profileRecord === undefined) return { state: "denied" };
      const latestGrant = parseCapabilityGrant(grantRecord), latestProfile = parseProfile(profileRecord);
      if (latestGrant === undefined || latestProfile === undefined) return { state: "unavailable" };
      if (latestGrant.client_id !== principal.client_id || latestProfile.profile_id !== profile.profile_id || !latestGrant.enabled || !latestProfile.enabled) return { state: "denied" };
