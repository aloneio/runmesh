...
async function selectedRelease(env: WorkerEnv, selection: ReleaseSelection, scheduleRefresh?: DevelopmentReleaseRefreshScheduler): Promise<RunnerReleaseDescriptor> {
  ...
  if (selection === "dev") return resolveDevelopmentRunnerRelease(env, developmentReleaseDependencies(registryDevelopmentReleaseCache(env)), scheduleRefresh);
  return resolveRunnerReleaseDescriptor(env, developmentReleaseDependencies(registryDevelopmentReleaseCache(env)), scheduleRefresh);
}
...
export async function runnerInstallScript(request: Request, url: URL, env: WorkerEnv, scheduleRefresh?: DevelopmentReleaseRefreshScheduler): Promise<Response> {
  ...
  const descriptor = await resolveRunnerReleaseDescriptor(env, developmentReleaseDependencies(registryDevelopmentReleaseCache(env)), scheduleRefresh); let content: string;
...
export async function runnerUninstallScript(request: Request, env: WorkerEnv, windows: boolean, scheduleRefresh?: DevelopmentReleaseRefreshScheduler): Promise<Response> {
  if (request.method !== "GET" && request.method !== "HEAD") { void discardBody(request); return methodNotAllowed("GET, HEAD"); }
  const descriptor = await resolveRunnerReleaseDescriptor(env, developmentReleaseDependencies(registryDevelopmentReleaseCache(env)), scheduleRefresh);
