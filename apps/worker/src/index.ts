...
import { RegistryDO, type McpClientRecord, type RunnerExecutionMode, type RunnerPublicInfo, type RunnerRecord, type VerifiedMcpClient } from "./registry.js";
...
async function handleBrowserAdmin(request: Request, env: WorkerEnv, url: URL): Promise<Response> {
  ...
  if (request.method === "GET" && runnerDetail !== null) {
    ...
    const [runnerResponse, workspaceResponse, jobsResponse, policyVersionsResponse, environment, releaseResponse] = await Promise.all([
    ...
    return runner === undefined ? adminError(404, "Runner was not found.") : html(adminDocument(`${typeof runner.display_name === "string" ? runner.display_name : runnerId} · Runner`, runnerDetailPage(runner, workspaces, jobs, environment, csrf, releaseResponse, policyVersions), "runners"));
  }
...
async function createBrowserRunner(env: WorkerEnv, form: FormData, baseUrl: string): Promise<Response> {
  ...
  let response: Response;
  try { response = await runnerRegistryRequest(env, runnerId, "/add", "POST", JSON.stringify({ display_name: displayName, mutation_id: mutationId, execution_mode: selection.mode, confirm_privileged_host: selection.confirmed })); }
  ...
  const codeResult = await createEnrollmentCode(env, runnerId, selection, { configuredMode: selection.mode, lifecycleId });
  if (!codeResult.ok) {
  ...
  return runnerEnrollmentPage(env, baseUrl, runnerId, code, String(form.get("csrf_token") ?? ""), false, selection.mode, selection.confirmed);
}
async function handleBrowserRunnerAction(env: WorkerEnv, form: FormData, baseUrl: string, runnerId: string, action: "rename" | "rotate" | "revoke" | "delete" | "enrollment" | "permissions" | "version-policy" | "emergency-lock" | "workspace-create" | "workspace-update" | "workspace-delete"): Promise<Response> {
  ...
  if (action === "rotate") {
    ...
    const codeResult = await createEnrollmentCode(env, runnerId, selection, { configuredMode: expectedConfiguredMode(fencedState.snapshot), lifecycleId: fencedState.snapshot.lifecycleId });
    if (!codeResult.ok) {
      if (codeResult.deterministic) {
        const settled = await settleRunnerMutation(env, runnerId, mutationId, true);
  ...
  if (action === "enrollment") {
    ...
    try { codeResult = await createEnrollmentCode(env, runnerId, selection, { configuredMode: expectedConfiguredMode(fencedState.snapshot), lifecycleId: fencedState.snapshot.lifecycleId }); }
...
async function createEnrollmentCode(env: WorkerEnv, runnerId: string, selection?: ExecutionModeSelection, expected?: RunnerExecutionExpectation): Promise<EnrollmentCodeResult> {
  ...
  return response.ok ? { ok: true, code } : { ok: false, status: response.status, deterministic: [400, 404, 409].includes(response.status) };
}
...
function runnerDetailPage(runner: Record<string, unknown>, workspaces: readonly unknown[], jobs: readonly unknown[], environment: Record<string, unknown> | undefined, csrf: string, release: RunnerReleaseDescriptor & { readonly distributable: boolean }, policyVersions: readonly unknown[] = []): string {
...
.admin-viewport{position:relative;max-width:1440px;margin:0 auto;min-height:160px;contain:layout paint}
.admin-page-container{position:absolute;inset:0;min-height:inherit;opacity:0;visibility:hidden;pointer-events:none;transform:translateY(5px);transition:opacity 160ms ease,transform 180ms ease,visibility 0s linear 180ms}
...
function setActivePage(container,focus){var viewport=ensurePageViewport();if(!viewport||!container)return;var containers=Array.prototype.slice.call(viewport.querySelectorAll('[data-page-container]'));containers.forEach(function(item){var active=item===container;item.classList.toggle('is-active',active);item.classList.remove('is-leaving');item.setAttribute('aria-hidden',active?'false':'true');var main=item.id==='main-content'?item:item.querySelector('#main-content');if(active){if(!main)main=item.tagName==='MAIN'?item:item.querySelector('main');if(main)main.id='main-content'}else if(main)main.removeAttribute('id')});viewport.style.minHeight=Math.max.apply(Math,[0].concat(containers.map(function(item){return item.offsetHeight||0})))+'px';if(focus){var main=container.id==='main-content'?container:container.querySelector('#main-content')||container.querySelector('main');if(main&&typeof main.focus==='function')main.focus({preventScroll:true})}}
...
function bindDynamicContent(root){
  if(!root)return;
  root.querySelectorAll('[data-copy]').forEach(function(button){if(button.__runmeshBound)return;button.__runmeshBound=true;button.addEventListener('click',function(){var result=copyText(button.getAttribute('data-copy')||'');var mark=function(){button.textContent=document.documentElement.lang==='zh-CN'?'已复制':'Copied';button.classList.add('copied')};if(result&&typeof result.then==='function')result.then(mark,function(){});else mark()})});
...
function loadAdminPage(url,shouldPush){var key=pageKey(url),viewport=ensurePageViewport(),existing=cachedPage(key);if(existing){if(shouldPush)history.pushState({runmeshAdmin:true},'',url.pathname+url.search+url.hash);document.title=existing.getAttribute('data-page-title')||document.title;setActivePage(existing,true);updateActiveNavigation(url.pathname);applyLocale(requestedLocale());return}if(window.__runmeshLoading){window.__runmeshQueuedUrl=url;window.__runmeshQueuedPush=shouldPush;return}window.__runmeshLoading=true;var current=document.querySelector('#main-content');if(current)current.setAttribute('aria-busy','true');fetch(url.pathname+url.search,{headers:{'X-Requested-With':'runmesh-spa'},credentials:'same-origin'}).then(function(response){if(!response.ok)throw new Error('HTTP '+response.status);return response.text()}).then(function(markup){var parsed=new DOMParser().parseFromString(markup,'text/html');var next=parsed.querySelector('#main-content');if(!next)throw new Error('main content missing');var nextRoot=pageRoot(next);if(!nextRoot)throw new Error('page root missing');nextRoot.removeAttribute('id');var container=pageContainer(nextRoot,key);container.setAttribute('data-page-title',parsed.title||'');viewport=ensurePageViewport();viewport.appendChild(container);document.title=parsed.title||document.title;bindDynamicContent(container);setupDynamicNavigation();if(shouldPush)history.pushState({runmeshAdmin:true},'',url.pathname+url.search+url.hash);setActivePage(container,true);updateActiveNavigation(url.pathname);applyLocale(requestedLocale())}).catch(function(error){console.error('Runmesh navigation failed',error);location.href=url.href}).finally(function(){window.__runmeshLoading=false;var active=document.querySelector('#main-content');if(active)active.removeAttribute('aria-busy');if(window.__runmeshQueuedUrl){var queued=window.__runmeshQueuedUrl,queuedPush=window.__runmeshQueuedPush;window.__runmeshQueuedUrl=null;loadAdminPage(queued,queuedPush)}})}
ensurePageViewport();var initialContainer=document.querySelector('[data-page-container].is-active');if(initialContainer){bindDynamicContent(initialContainer);setActivePage(initialContainer,false)}
...
export function runnerEnrollmentPage(env: RunnerReleaseEnvironment, baseUrl: string, runnerId: string, code: string | undefined, csrf: string, _reEnroll = false, executionMode: ConsoleExecutionMode = "dedicated_user", confirmPrivilegedHost = false): Response {
...
printf '\\n' >&2
printf '%s\\n' "$RUNMESH_ENROLLMENT_CODE" | sudo "$RUNNER" enroll --server ${shellServer} --code-stdin ${modeFlags}
