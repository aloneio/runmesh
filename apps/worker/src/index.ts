...
const ZH_UI_TEXT: Record<string, string> = {
  ...
  "The installer verifies the fixed signed Runner artifact, installs missing Node.js and npm tools when supported, registers the Runner as a background service, and starts it after enrollment. It asks for this one-time code locally and never places the code in this command, a URL, or process arguments.": "安装器会校验固定签名的 Runner；在支持的系统上自动补齐 Node.js 和 npm，注册后台服务，并在注册成功后自动启动。它会在本机提示输入一次性代码，代码不会写入命令、URL 或进程参数。",
...
if ($LASTEXITCODE -ne 0) { throw 'Runner doctor check failed.' }`,
  ...
  const instruction = commands === manualCommands
    ...
    : `The installer verifies the fixed signed Runner artifact, installs missing Node.js and npm tools when supported, registers the Runner as a background service, and starts it after enrollment. It asks for this one-time code locally and never places the code in this command, a URL, or process arguments. Selected execution mode: ${modeLabel}. The recommended execution mode is privileged_host; dedicated_user remains available for explicit isolation cases.`;
