...
async function handleRequest(request: Request, env: WorkerEnv, _ctx: ExecutionContext): Promise<Response> {
  ...
  if (url.pathname === "/runner/releases/latest") return runnerRelease(request, env);
...
export function runnerReleaseDescriptor(env: Pick<WorkerEnv, "RUNNER_PACKAGE_SPEC" | "RUNNER_PACKAGE_NAME" | "RUNNER_PACKAGE_VERSION">): RunnerReleaseDescriptor & { readonly distributable: boolean } {
  ...
  return {
    package_name: configuredName ?? CURRENT_RUNNER_PACKAGE_NAME,
    package_version: configuredVersion ?? CURRENT_RUNNER_PACKAGE_VERSION,
    package_spec: "",
    distributable: false,
    protocol: { min_version: PROTOCOL_MIN_VERSION, max_version: PROTOCOL_CURRENT_VERSION },
  };
...
function validRunnerPackageSpec(value: string | undefined): string | undefined {
  ...
  try {
    ...
    return url.protocol === "https:" && url.username === "" && url.password === "" && url.search === "" && url.hash === "" && !url.hostname.endsWith("github.com") && url.pathname.endsWith(".tgz") ? url.toString() : undefined;
...
case "$(uname -s)" in
  Linux) PROFILE="$HOME/.remote-coding-runner/profile.json"; MANIFEST="$HOME/.config/systemd/user/coding-runner.service"; ACTIVATION_ONE='systemctl --user daemon-reload'; ACTIVATION_TWO='systemctl --user enable coding-runner.service';;
  Darwin) PROFILE="$HOME/Library/Application Support/RemoteCodingRunner/profile.json"; MANIFEST="$HOME/Library/LaunchAgents/com.remote-coding.runner.plist"; ACTIVATION_ONE='launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.remote-coding.runner.plist'; ACTIVATION_TWO='';;
...
if [ "$(id -u)" -eq 0 ]; then die "refusing to install as root; run this as the intended user"; fi
...
NPM_PREFIX="$HOME/.local/share/remote-coding-runner/npm"
mkdir -p "$NPM_PREFIX"
npm install --global --prefix "$NPM_PREFIX" "$PACKAGE_SPEC"
PATH="$NPM_PREFIX/bin:$PATH"; export PATH
command -v coding-runner >/dev/null 2>&1 || die "npm did not install coding-runner into the per-user prefix"
coding-runner enroll --server "$SERVER_ORIGIN/runner/enroll" --code "$CODE"
coding-runner install
printf '%s\\n' 'Runner service manifest installed. Review and run the following activation command(s):'
printf '%s\\n' "$ACTIVATION_ONE"
if [ -n "$ACTIVATION_TWO" ]; then printf '%s\\n' "$ACTIVATION_TWO"; fi
`;
...
$ServerOrigin = '${origin}'
$ProfilePath = Join-Path $env:LOCALAPPDATA 'RemoteCodingRunner\\profile.json'
$ManifestPath = Join-Path $env:LOCALAPPDATA 'RemoteCodingRunner\\coding-runner-task.xml'
...
$NpmPrefix = Join-Path $env:LOCALAPPDATA 'RemoteCodingRunner\\npm'
New-Item -ItemType Directory -Force -Path $NpmPrefix | Out-Null
& npm install --global --prefix $NpmPrefix $Release.package_spec
$env:Path = "$NpmPrefix;$env:Path"
if (-not (Get-Command coding-runner -ErrorAction SilentlyContinue)) { throw 'npm did not install coding-runner into the per-user prefix.' }
coding-runner enroll --server "$ServerOrigin/runner/enroll" --code $EnrollmentCode
coding-runner install
Write-Output 'Runner Scheduled Task manifest installed. Review and run this per-user activation command:'
Write-Output "schtasks /Create /TN RemoteCodingRunner /XML \`"$ManifestPath\`" /F"
`;
...
async function handleBrowserAdmin(request: Request, env: WorkerEnv, url: URL): Promise<Response> {
  ...
  if (request.method === "GET" && runnerDetail !== null) {
    ...
    const [runnerResponse, workspaceResponse, jobsResponse, environment] = await Promise.all([
      registryGet(env, `/runners/${encodeURIComponent(runnerId)}`),
      registryGet(env, `/runners/${encodeURIComponent(runnerId)}/workspaces`),
    ...
    return runner === undefined ? adminError(404, "Runner was not found.") : html(runnerDetailPage(runner, workspaces, jobs, environment));
  }
  ...
  const runnerMatch = /^\/admin\/runners\/([A-Za-z0-9][A-Za-z0-9._:-]{0,127})\/(rename|rotate|revoke|delete|enrollment)$/.exec(url.pathname);
  if (runnerMatch !== null) return handleBrowserRunnerAction(env, form, request.url, runnerMatch[1] as string, runnerMatch[2] as "rename" | "rotate" | "revoke" | "delete" | "enrollment");
...
async function handleBrowserRunnerAction(env: WorkerEnv, form: FormData, baseUrl: string, runnerId: string, action: "rename" | "rotate" | "revoke" | "delete" | "enrollment"): Promise<Response> {
  if (action === "rename") {
...
function adminPage(pathname: string, data: AdminData, csrf: string): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(active[0]?.toUpperCase() ?? "Dashboard")} · Remote Coding Runtime</title>${adminStyles()}</head><body><div class="shell"><header class="topbar"><a class="brand" href="/admin">Remote Coding Runtime</a><div class="top-actions"><a class="button secondary" href="/admin/runners#add-runner">Add Runner</a><a class="button" href="/admin/clients#add-client">Add MCP Client</a></div></header>${nav}<main>${body}</main></div>${adminScript()}</body></html>`;
}
...
function runnerDetailPage(runner: Record<string, unknown>, workspaces: readonly unknown[], jobs: readonly unknown[], environment: Record<string, unknown> | undefined): string {
...
function runnerEnrollmentPage(baseUrl: string, runnerId: string, code: string | undefined, csrf: string): Response {
  if (code === undefined) return adminError(503, "Enrollment code could not be generated.");
  const installSh = `curl -fsSL ${new URL("/runner/install.sh", baseUrl).toString()} | sh -s -- ${code}`;
  const installPs1 = `& ([scriptblock]::Create((Invoke-RestMethod -Uri '${new URL("/runner/install.ps1", baseUrl).toString()}'))) -EnrollmentCode '${code}'`;
...
function throttleError(retryAfterMs: number): Response {
  ...
  return new Response("<!doctype html><title>Remote Coding Runtime</title><main><h1>Remote Coding Runtime</h1><p>Invalid administrator password.</p><p>Please try again shortly.</p></main>", { status: 403, headers });
}
