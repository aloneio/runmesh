...
// Keep the application chrome mounted while admin pages stream into the workspace.
...
function loadAdminPage(url,shouldPush){if(window.__runmeshLoading)return;window.__runmeshLoading=true;var current=document.querySelector('#main-content');if(current)current.setAttribute('aria-busy','true');fetch(url.pathname+url.search,{headers:{'X-Requested-With':'runmesh-spa'},credentials:'same-origin'}).then(function(response){if(!response.ok)throw new Error('HTTP '+response.status);return response.text()}).then(function(markup){var parsed=new DOMParser().parseFromString(markup,'text/html');var next=parsed.querySelector('#main-content');if(!next)throw new Error('main content missing');var old=document.querySelector('#main-content');var oldRoot=old&&old.closest('.shell')||old;var nextRoot=next.classList.contains('shell')?next:(next.closest('.shell')||next);if(oldRoot)oldRoot.replaceWith(nextRoot);if(shouldPush)history.pushState({runmeshAdmin:true},'',url.pathname+url.search+url.hash);document.title=parsed.title||document.title;updateActiveNavigation(url.pathname);applyLocale(requestedLocale());bindDynamicContent(nextRoot);setupDynamicNavigation();next.focus({preventScroll:true})}).catch(function(error){console.error('Runmesh navigation failed',error);location.href=url.href}).finally(function(){window.__runmeshLoading=false;var active=document.querySelector('#main-content');if(active)active.removeAttribute('aria-busy')})}
