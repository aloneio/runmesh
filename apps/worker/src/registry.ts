...
export class RegistryDO {
  ...
  public registerRunner(runnerId: string, tokenVerifier: string, nowMs: number, mutationId?: string): boolean {
    ...
    return this.ctx.storage.transactionSync(() => {
      ...
      if (mutationId !== undefined) {
        if (existing === undefined || !validMutationId(mutationId)) return false;
      ...
      this.ctx.storage.transactionSync(() => {
        this.ctx.storage.sql.exec(
          `INSERT INTO runners (runner_id, display_name, token_verifier, state, management_mode, credential_version, lifecycle_id, desired_policy_revision, desired_policy_checksum, policy_status, runner_permissions_json, updated_at_ms)
           VALUES (?, ?, '', 'offline', 'central', 0, ?, 0, NULL, 'pending', ?, ?)`, runnerId, displayName, crypto.randomUUID(), JSON.stringify(READ_ONLY_PERMISSIONS), nowMs,
        );
  ...
  public async redeemRunnerEnrollment(verifier: string, tokenVerifier: string, publicInfo: RunnerPublicInfo, nowMs: number, mutationId?: string): Promise<{ runner_id: string } | undefined> {
    ...
    return this.ctx.storage.transactionSync(() => {
      ...
      if (mutationId !== undefined) {
        ...
        const recorded = this.recordCredentialMutation(row.runner_id, mutationId, "credential_rotate", nowMs);
        if (recorded === "conflict") return undefined;
        if (recorded === "committed") {
          // A retried mutation can only observe this branch after a legacy
          // partial record. Consume the code now so it cannot be redeemed a
          // second time with a different mutation id.
  ...
  public async fetch(request: Request): Promise<Response> {
    ...
    if (request.method === "PUT" && action === undefined) {
      ...
      if (mutationId !== undefined && existingRunner === undefined) return new Response("not found", { status: 404 });
  ...
  private mutationRow(runnerId: string, mutationId: string): { kind: "credential_rotate" | "credential_revoke" | "runner_delete"; pre_credential_version: number; lifecycle_id: string } | undefined {
    return this.ctx.storage.sql.exec<{ kind: "credential_rotate" | "credential_revoke" | "runner_delete"; pre_credential_version: number; lifecycle_id: string }>(
  ...
  private recordCredentialMutation(runnerId: string, mutationId: string, kind: "credential_rotate" | "credential_revoke" | "runner_delete", nowMs: number): "new" | "committed" | "conflict" {
    const runner = this.runnerRow(runnerId);
    ...
    if (existing !== undefined) {
      ...
      return existing.kind === "runner_delete"
        ? "new"
        : runner.credential_version !== existing.pre_credential_version ? "committed" : "new";
    }
