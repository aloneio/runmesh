...
export class RunnerDO {
  ...
  private async recoverCredentialMutation(expected: AdmissionState): Promise<boolean> {
    ...
    return state.runner_exists === true && state.mutation_committed === true
      ...
      // field remains compatible, but an explicit mismatch must never recover
      // a credential mutation into a different runner lifecycle.
      && (expected.lifecycleId === null || state.lifecycle_id === undefined || state.lifecycle_id === expected.lifecycleId);
  }
