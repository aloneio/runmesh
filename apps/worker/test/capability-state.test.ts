...
const read = (stub: ReturnType<typeof owner>, id: string) => runInDurableObject(stub, (_instance, state) => new CapabilityState(state.storage).readGrant(id));
const replace = (stub: ReturnType<typeof owner>, input: GrantReplacement) => runInDurableObject(stub, (_instance, state) => new CapabilityState(state.storage).replaceGrant(input));
// Legacy storage remains readable for migration; no live grant RPC is exposed.
...
it("W03 central records use their own namespace and never require a Runner", async () => {
  ...
  expect(await replace(stub, create)).toMatchObject({ state: "written", grant: { revision: 1 } });
  expect(await read(other, create.client_id)).toBeUndefined();
  expect(await read(stub, create.client_id)).toMatchObject({ client_id: create.client_id, enabled: true, rules: create.rules });
  await runInDurableObject(stub, (_instance, state) => {
...
it("W03 concurrent replacements require the observed revision and preserve disable history", async () => {
  const stub = owner();
  const results = await Promise.all([replace(stub, create), replace(stub, create)]);
  expect(results.map(result => result.state).sort()).toEqual(["conflict", "written"]);
  expect(await replace(stub, { ...create, expected_revision: 1, enabled: false })).toMatchObject({ state: "written", grant: { revision: 2, enabled: false, rules: create.rules } });
  expect(await replace(stub, create)).toEqual({ state: "conflict", current_revision: 2 });
});

it("W03 unknown schema versions are not reset or silently downgraded", async () => {
  const stub = owner();
  await replace(stub, create);
  await runInDurableObject(stub, (_instance, state) => {
...
it("W03 grant capacity is bounded while disabling an existing record remains possible", async () => {
  const stub = owner();
  await read(stub, create.client_id);
  await runInDurableObject(stub, (_instance, state) => {
  ...
  expect(await replace(stub, create)).toEqual({ state: "capacity" });
  expect(await replace(stub, { ...create, client_id: "client-1", expected_revision: 1, enabled: false })).toMatchObject({ state: "written" });
});

it.each([-1, 0.5, Number.MAX_SAFE_INTEGER])("W03 rejects unsafe expected revision %s", async expected_revision => {
  const stub = owner();
  expect(await replace(stub, { ...create, expected_revision })).toEqual({ state: "invalid" });
  await runInDurableObject(stub, (_instance, state) => {
    expect(state.storage.sql.exec("SELECT name FROM sqlite_master WHERE name='capabilities_meta'").toArray()).toEqual([]);
  });
});
