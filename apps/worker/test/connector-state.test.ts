...
const record: ProfileRecord = { profile: { schema_version: 1, profile_id: "profile-a", connector_id: "docs", endpoint: "https://docs.example/mcp",
  owner: { kind: "instance_admin" }, revision: 1, enabled: false, credential: { secret_id: "profile-a", secret_version: 1 } },
  envelope: { schema_version: 1, key_id: "test-only", iv: "A".repeat(16), ciphertext: "A".repeat(40) } };
function owner() {
...
const store = (state: DurableObjectState) => new ConnectionState(state.storage, () => new CapabilityState(state.storage).initialize());

it("W03 connection repository is lazy and keeps grants in their original tables", async () => {
  await runInDurableObject(owner(), (_instance, state) => {
    ...
    const grants = new CapabilityState(state.storage);
    expect(grants.replaceGrant({ client_id: "client-a", expected_revision: 0, enabled: true, rules: [] })).toMatchObject({ state: "written" });
    expect(store(state).read("profile-a")).toEqual(record);
    expect(grants.readGrant("client-a")).toMatchObject({ revision: 1 });
  });
});
...
it("W03 observed revisions prevent overwrites and destination changes", async () => {
  await runInDurableObject(owner(), (_instance, state) => {
    ...
    expect(repository.replace({ ...next, envelope: { ...record.envelope, ciphertext: "B".repeat(40) } }, 1)).toEqual({ state: "invalid" });
...
it("W03 schema upgrades are forward-only and never clear unknown version data", async () => {
  await runInDurableObject(owner(), (_instance, state) => {
    ...
    expect(new CapabilityState(state.storage).readGrant("absent")).toBeUndefined();
