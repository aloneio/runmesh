...
describe("MCP tool catalog", () => {
  it("keeps the stable public tool surface in one catalog", () => {
    expect(Object.keys(TOOL_SPECS).sort()).toEqual([
      "context", "edit", "inspect", "job", "read", "runner_current", "runner_list", "runner_select", "shell", "workspace_list",
    ]);
  ...
  it("rejects unknown fields and preserves action-specific schemas", () => {
    ...
    expect(TOOL_SPECS.context.inputSchema.safeParse({ action: "checkpoint", workspace_id: "workspace", turn_id: "turn-1", goal: "handoff", evidence: [{ kind: "job" }] }).success).toBe(false);
    expect(TOOL_SPECS.context.inputSchema.safeParse({ action: "search", workspace_id: "workspace", query: "handoff" }).success).toBe(true);
  });
});
