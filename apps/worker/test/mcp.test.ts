...
describe.sequential("self-hosted admin and MCP client authentication", () => {
  ...
  it("renders immutable signed installers with embedded key verification and no code input surface", () => {
    ...
    expect(shell).toContain("--code-stdin"); expect(shell).toContain("/dev/tty"); expect(shell).toContain("stty -echo"); expect(shell).toContain("FINAL=\"$INSTALL_ROOT/versions/$VERSION\""); expect(shell).toContain("ENROLLMENT_ATTEMPTED=0"); expect(shell).toContain("trap on_exit EXIT"); expect(shell).toContain("trap 'rollback 1' HUP INT TERM"); expect(shell).toContain("command_name in curl tar gzip"); expect(shell).toContain("check_base_tools"); expect(shell).toContain("check_service_prerequisites"); expect(shell).toContain("node-v22.23.2"); expect(shell).toContain("NODE_SHA256"); expect(shell).not.toContain("ENROLLMENT_INPUT"); expect(shell).not.toContain("REFRESH_INPUT"); expect(shell).toMatch(/printf '[^']*' \"\$ENROLLMENT_CODE\" \|/); expect(shell).toContain('"$NODE" "$NPM_CLI"'); expect(shell).toContain("--ignore-scripts --offline");
  ...
  it("sets up once, authenticates a session, enforces CSRF, and manages independent secret URLs", async () => {
    ...
    const rejectedCsrf = await submit("https://worker.test/setup", { password, confirm_password: password }, new Map(), false);
    expect(rejectedCsrf.status).toBe(403);

    const rejectedSetup = await submit("https://worker.test/setup", { csrf_token: setupCsrf }, jar([["__Host-runmesh_setup_csrf", setupCookie]]));
    ...
    const setupRequests = await Promise.all(Array.from({ length: 2 }, () => submit(
      ...
      { csrf_token: setupCsrf, password, confirm_password: password },
    ...
    expect(setup?.headers.get("set-cookie")).toContain("__Host-runmesh_setup_csrf=");
