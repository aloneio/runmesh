...
8. **Operator-mediated local installation.** Dashboard enrollment renders public bootstrap commands and Runner service actions render manifest instructions; they do not execute host service activation. New machine profiles use dedicated-user service manifests; privileged host execution requires explicit confirmation. The local CLI preserves legacy profiles and per-user compatibility. Distribution remains operator-configured through an exact stable package spec or HTTPS tarball.
