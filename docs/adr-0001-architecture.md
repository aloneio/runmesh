...
8. **Operator-mediated local installation.** Dashboard enrollment renders a manual portable-artifact enrollment command and Runner service actions render manifest instructions; they do not execute host service activation. In `v0.1.0-dev.2`, hosted bootstrap is unavailable and its scripts fail closed. `coding-runner install` invokes the Runmesh service provisioner for Runmesh-owned identities/directories and Windows Local Service ACLs; it never changes Workspace ownership or modes. New machine profiles use dedicated-user service manifests; privileged host execution requires explicit confirmation. Profiles missing execution-mode or management metadata are surfaced as `migration_required` and require an explicit operator migration choice; per-user compatibility remains opt-in.

## Consequences

The runtime provides protocol negotiation, correlated `request_id` exchanges, Worker/Runner transport, Durable Object coordination, Runner enrollment, admin/client authentication, profile-backed CLI operations, sticky Runner routing, and real local E2E coverage. The Registry performs additive startup schema repairs for known older columns/tables/indexes, but there is no standalone versioned migration/downgrade tool. The dashboard renders the supported `coding-runner enroll --server ... --code-stdin` syntax but does not execute host commands; public bootstrap installers and hosted release artifacts remain deferred.
