...
8. **Operator-mediated local installation.** Dashboard enrollment renders a code-free one-command installer only when the exact fixed signed preview is explicitly enabled after release publication and verification; otherwise it renders the portable-artifact route and `--code-stdin`. The installer is trust model A: Worker HTTPS delivery is its bootstrap root, while an embedded reviewed key verifies fixed GitHub release assets. It accepts no arbitrary package/URL/moving version and downloaded keyrings are not trust roots. `coding-runner install` invokes the Runmesh service provisioner for Runmesh-owned identities/directories and Windows Local Service ACLs; it never changes Workspace ownership or modes. New machine profiles use dedicated-user service manifests; privileged host execution requires explicit confirmation. The local CLI preserves legacy profiles and per-user compatibility.

## Consequences

The runtime provides protocol negotiation, correlated `request_id` exchanges, Worker/Runner transport, Durable Object coordination, Runner enrollment, admin/client authentication, profile-backed CLI operations, sticky Runner routing, and real local E2E coverage. The Registry performs additive startup schema repairs for known older columns/tables/indexes, but there is no standalone versioned migration/downgrade tool. The dashboard renders a code-free fixed installer command only when the exact signed preview assets are explicitly enabled; otherwise it renders the portable-artifact route and `coding-runner enroll --server ... --code-stdin`. It never executes host commands or embeds the code in copied commands. Hosted release publication remains a separately authorized operation.
