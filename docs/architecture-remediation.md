...
Development targets `runmeshdev`, production `runmesh`. Before uploading, the deployment wrapper checks the config and `WRANGLER_CI_OVERRIDE_NAME` against the branch/environment target and passes an explicit Worker name. This does not authenticate a different account or attest the dashboard trigger configuration. Main promotion, live acceptance and immutable signed publication remain separate actions.
