...
启用 Skills、直接目录和治理开关；配置未预置远程端点或密钥环，受管连接通过控制端保存目标策略，仅 OAuth 需要密钥库；旧档案需显式出站配置。生产配置不变；本说明不代表正式发版或通过真实公网服务验收。
...
原 W05 基线尚不支持 OAuth 刷新／提权、任意请求头、查询参数凭据、stdio 托管、长期会话、
...
变量名为 `CENTRAL_MCP_EGRESS`，仅旧档案在缺失或格式错误时不开放远程入口。不接受非标准
