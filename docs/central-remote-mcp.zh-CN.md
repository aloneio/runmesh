...
W03 凭据档案及 W04 审核目录。现有生产／development 部署配置仍没有中央绑定或
出站策略；本说明不表示已经上线、发版或完成真实公网服务验收。
...
只有 CAPABILITIES 与有效出站策略同时存在，才注册这两个中央工具。构造服务和
只调用原生工具不会解析中央存储、读取密钥或连接上游，中央故障不会移除原生工具。
...
工具 ID、版本和连接档案权限。授权管理界面仍属于 W08；身份存在、档案启用或目录
批准，都不等于客户端已经得到工具权限。
...
审计表写入参数／结果正文，也不留下请求生命周期以外的定时器。持久化的元数据调用
回执、速率限制和更广泛成本验证仍属于后续验收。
