...
4. The authenticated Admin enrollment page displays one of two routes. It labels the fixed signed preview installer as available only after the exact immutable release has been published, independently verified, and explicitly enabled for this Worker. That OS-specific command carries the single-use enrollment code as its final argument; after fixed-asset verification and staging, the installer forwards it only through `runmesh enroll --code-stdin` and clears its working variable. Otherwise use the manual [portable Runner verification and installation procedure](portable-runner-installation.md). Treat the copied command as a secret and use it only once.
