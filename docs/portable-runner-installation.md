...
Runmesh 0.1.1 is a **published immutable stable patch release**. The explicit `development` and `test` environments keep distribution disabled, while the checked-in top-level production configuration and named `production` alias acknowledge the independently verified `v0.1.1` release with `RUNMESH_SIGNED_RELEASE_AVAILABLE=0.1.1` and its canonical HTTPS `RUNMESH_PUBLIC_ORIGIN`. Both dedicated_user and privileged_host retain one-command enrollment. A deployment that has not yet picked up the enabled production configuration must use this independently verified portable route. The bundled runtime is Node 22.23.2; supported external runtimes are Node 22.23.2+ (22.x) and 24.21.0+ (24.x).
...
- version `0.1.1` and tag `v0.1.1`;
- the exact GitHub release-asset URLs and `runmesh-runner-0.1.1.tgz` name;
