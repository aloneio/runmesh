...
> **Enrollment behavior:** hosted `/runner/install.sh` and `/runner/install.ps1` remain fail-closed until the exact fixed signed preview release has been published, independently verified, explicitly enabled for the Worker, **and a valid canonical external HTTPS `RUNMESH_PUBLIC_ORIGIN` is configured**. When disabled, they do not accept or consume a one-time code, access the network, invoke npm, or activate a host service; the dashboard shows the independently verified portable-artifact flow. When enabled, the dashboard displays an installer command whose origin is the configured public origin and whose final argument is the single-use enrollment code. The installer verifies fixed release assets and stages both Runner entry points before forwarding the code only through `--code-stdin`, then clears its working variable. No generated command contains a token, Runner ID, MCP URL, Workspace root, arbitrary package spec, or moving release URL.
...
A profile holds the connection URL, Runner ID, token, workspace records, optional job concurrency, and optional execution mode. It is saved under the per-platform locations documented in [deployment.md](deployment.md). Ordinary/user POSIX storage uses private `0700`/`0600` directory/file modes; a dedicated system service intentionally uses the exact `0750`/`0640` shape within its root/service-group boundary so the service account can read the credential, with no group/other write bits. Low-level/manual profiles default to `dedicated_user` and central Workspace management; the authenticated dashboard's standard Add Runner flow explicitly selects `privileged_host` after the visible confirmation. Profiles that omit `execution_mode` or `management_mode` are reported as `migration_required`: a missing `execution_mode` blocks system installation until an explicit `--execution-mode` is supplied, while a missing `management_mode` leaves `workspace list` readable but blocks local Workspace mutation until `workspace migrate --management-mode central` or `workspace migrate --management-mode legacy_manual` records the choice. The CLI does not auto-discover or import legacy profile/state paths; point it at a reviewed profile explicitly for manual migration. `status` redacts the token; `doctor --json` returns stable required/optional diagnostics for profile directory/file permissions, service manifest/installed/active state, Host shell, execution mode, local policy revision when available, and tools; missing Python/Docker are warnings. `env` runs bounded local discovery, and `start` defaults to the profile but continues to support explicit legacy transport/workspace flags.

The service adapter writes marked and content-hashed system manifests and refuses to overwrite/remove an unmarked or changed file. `runmesh install` invokes the Runmesh service provisioner, which creates the dedicated Runmesh account/group and Runmesh-owned directories on Linux and macOS, and applies Local Service ACLs to Runmesh-owned install/config/state/log/profile paths on Windows. Dedicated-user Linux units explicitly set `User=runmesh` and `Group=runmesh`; macOS LaunchDaemons set `UserName=runmesh`; Windows tasks use `NT AUTHORITY\LOCAL SERVICE` at least privilege. The provisioner never changes Workspace ownership or modes; operators grant the service identity only the minimum required Workspace access. The dashboard's standard machine flow emits `--execution-mode privileged_host --confirm-privileged-host` after its explicit warning; direct CLI/manual installation remains dedicated-user unless those flags are supplied. Profiles without an execution mode require an explicit choice and are not silently migrated. Hosted bootstrap is implemented as a fixed signed-preview mechanism and remains disabled by default in the default/local environment. The checked-in `production` environment carries the gates for the independently verified immutable release; a canonical `RUNMESH_PUBLIC_ORIGIN` and the exact `RUNMESH_SIGNED_RELEASE_AVAILABLE=0.1.0-dev.2` acknowledgement are both required before it is exposed. Until that environment is deployed, use a manually verified portable artifact and `runmesh enroll --server ... --code-stdin` followed by `runmesh install`. The one-command Worker script is the bootstrap trust root; use an independent keyring/offline path for high assurance. The dashboard/service-action pages render commands and manifests but do not activate host services themselves.
