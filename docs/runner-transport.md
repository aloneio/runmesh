...
> **Enrollment behavior:** hosted `/runner/install.sh` and `/runner/install.ps1` remain fail-closed until the exact fixed signed release release has been published, independently verified, explicitly enabled for the Worker, **and a valid canonical external HTTPS `RUNMESH_PUBLIC_ORIGIN` is configured**. When disabled, they do not accept or consume a one-time code, access the network, invoke npm, or activate a host service; the dashboard shows the independently verified portable-artifact flow. When enabled, the dashboard displays an installer command whose origin is the configured public origin and which includes the quoted single-use enrollment code for automatic registration after verification. The installer verifies fixed release assets and stages both Runner entry points before forwarding the code only through `--code-stdin`, then clears its working variable. No generated command contains a long-lived Runner token, Runner ID, MCP URL, Workspace root, arbitrary package spec, or moving release URL. The one-time enrollment code is intentionally present in hosted commands; keep the complete command private.
