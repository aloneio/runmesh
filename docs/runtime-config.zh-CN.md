...
生产身份由环境确定；公网地址取自校验后的 HTTPS 请求 URL 和匹配的 Host，不使用转发头推测；历史记录默认使用生产 D1 绑定。只有反向代理等特殊部署才需要显式配置 `RUNMESH_PUBLIC_ORIGIN`。

当前 **0.1.4 是候选版本，默认关闭 stable 安装分发**。正式安装器是否可用取决于经过独立验证的发布记录，不能只凭包版本或手填变量启用。已发布的不可变 v0.1.3 资产保持原样。开发环境默认发现并验签当前系列的 dev 预发布，发现失败不会退回 stable。任何环境中，显式空的 `RUNMESH_SIGNED_RELEASE_AVAILABLE` 都会关闭托管安装。

旧的身份、域名、后端和发布门控变量仍作为兼容覆盖项。显式空值或错误值不会被自动改成允许状态。删除生产 D1 绑定不会使历史写入自动回退到核心 DO。

构建时核对实际 Git 提交和文件树，运行时再与 Cloudflare 版本元数据比较。标签缺失时仍可报告已确认的编译来源；脏工作区、来源缺失或冲突保持不可确认，不按版本号猜提交。参见[构建来源核验](build-provenance.zh-CN.md)。
...
发布签名密钥仅属于 GitHub 发布环境；Cloudflare API 凭据仅属于部署连接或授权 CLI，均不应放在 Worker 运行时。正式发版和生产部署使用受保护的 `main`，生产部署脚本还会拒绝尚未激活的候选状态；开发部署使用独立的 `dev` Worker。更新 Worker 不会升级主机上已安装的 Runner。
