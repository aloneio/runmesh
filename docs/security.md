...
The implemented service adapter writes only marked, content-hashed per-user manifest files and refuses to overwrite/remove an unmarked or changed file. It renders `systemctl`, `launchctl`, or `schtasks` commands for operators; `install` writes a manifest but does not execute host service activation, and `stop`/`restart`/`uninstall` do not execute their reported host command. Public bootstrap endpoints are secret-free and require an operator-configured stable package spec; no package is published by this repository.
