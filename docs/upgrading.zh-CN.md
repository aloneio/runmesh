...
当前开发版中央能力库升级后，所有现有有效客户端共享已启用、已审核的 MCP 工具和当前 Skill。旧的逐客户端规则不再限制访问，相关管理接口返回 HTTP 410。部署后需刷新缓存目录与 Skill 摘要。撤回单个客户端时撤销其凭证；撤回能力时全局禁用该能力。Runner 原生权限仍独立生效。曾使用逐客户端能力分配的实例，部署前请阅读[中央能力迁移说明](central-administration.zh-CN.md)。
