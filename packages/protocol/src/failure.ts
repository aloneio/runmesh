...
export function failureMetadata(code: string, observedState?: RpcOperationState): RpcFailureMetadata {
  ...
  else if (code === "file_changed" || code === "log_changed") result = { failure_class: "conflict", operation_state: "not_started", next_action: "re_read_and_retry" };
  else if (code === "read_budget_exhausted") result = { failure_class: "resource", operation_state: "not_started", next_action: "correct_request" };
