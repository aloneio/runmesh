...
export function failureMetadata(code: string, observedState?: RpcOperationState): RpcFailureMetadata {
  ...
  else if (code === "log_unavailable") result = { failure_class: "availability", operation_state: "not_started", next_action: "inspect_job" };
  else if (["file_changed", "log_changed", "cursor_mismatch", "cursor_expired"].includes(code)) result = { failure_class: "conflict", operation_state: "not_started", next_action: "re_read_and_retry" };
  else if ((code === "read_budget_exhausted" || code === "snapshot_too_large")) result = { failure_class: "resource", operation_state: "not_started", next_action: "correct_request" };
  else if (["baseline_changed", "expected_hash_mismatch", "hunk_ambiguous", "hunk_not_found", "hunk_overlap", "target_exists", "missing_file", "search_snapshot_changed", "request_id_conflict", "context_revision_conflict", "context_turn_conflict", "context_plan_changed"].includes(code)) result = { failure_class: "conflict", operation_state: "not_started", next_action: "re_read_and_retry" };
  else if (["file_too_large", "git_output_too_large", "context_record_too_large", "context_index_too_large", "context_rebuild_budget", "context_storage_full", "context_scan_budget", "queue_full", "busy"].includes(code)) result = code === "busy" || code === "queue_full" ? { failure_class: "resource", operation_state: "not_started", retry_after_ms: 1_000, next_action: "wait_and_retry" } : { failure_class: "resource", operation_state: "not_started", next_action: "correct_request" };
  ...
  else if (code === "authorization_response_invalid") result = { failure_class: "internal", operation_state: "not_started", next_action: "contact_operator" };
  else if (code === "internal_error" || code === "runner_rpc_failed" || code === "context_result_invalid") result = { failure_class: "internal", operation_state: "unknown", next_action: "contact_operator" };
