...
export function captureBuildProvenance(root, metadata = process.env) {
  ...
  try {
    ...
    if (top.status !== 0 || realpathSync(top.stdout.trim()) !== realpathSync(root)) return absent("unavailable", "git_unavailable");
    ...
    if (git("rev-parse", "HEAD").stdout.trim() !== commit || !clean()) return absent("dirty", "source_changed");
